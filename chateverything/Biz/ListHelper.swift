...
struct ListHelperParams {
    ...
    init(
        page: Int = 1, pageSize: Int = 20, sorts: [String: String] = [:],
        search: [String: Any] = [:]
    ) {
        self.page = page
...
class ListHelperEvents<T> {
    ...
    init(
        onLoaded: @escaping (ListHelperParams, [T]) -> Void,
        onLoading: @escaping (ListHelperParams) -> Void,
        onError: @escaping (ListHelperParams, Error) -> Void
    ) {
